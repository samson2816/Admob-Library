...
public class AdUtilsLogEventManager {

    private static final String TAG = "AdUtilsLogEventManager";
    ...
    public static void logPaidAdImpression(Context context, AdValue adValue, String adUnitId, String mediationAdapterClassName, AdType adType) {
        logEventWithAds(context, (float) adValue.getValueMicros(), adValue.getPrecisionType(), adUnitId, mediationAdapterClassName, AdUtilsConfig.PROVIDER_ADMOB);
        AdUtilsAdjust.pushTrackEventAdmob(adValue);
        AdUtilsAppsflyer.getInstance().pushTrackEventAdmob(adValue, adUnitId, adType);
    }

    public static void logPaidAdImpression(Context context, MaxAd adValue, AdType adType) {
//        logEventWithAds(context, (float) adValue.getRevenue(), 0, adValue.getAdUnitId(), adValue.getNetworkName(), AdUtilsConfig.PROVIDER_MAX);
        logEventWithMaxAds(context, adValue);
        AdUtilsAdjust.pushTrackEventApplovin(adValue, context);
        AdUtilsAppsflyer.getInstance().pushTrackEventApplovin(adValue, adType);
    }
    ...
    private static void logPaidAdImpressionValue(Context context, double value, int precision, String adunitid, String network, int mediationProvider) {
        ...
        AdUtilsAdjust.logPaidAdImpressionValue(value, "USD");
    ...
    public static void setEventNamePurchaseAdjust(String eventNamePurchase) {
        AdUtilsAdjust.setEventNamePurchase(eventNamePurchase);
    }

    public static void trackAdRevenue(String id) {
        AdUtilsAdjust.trackAdRevenue(id);
    }

    public static void onTrackEvent(String eventName) {
        AdUtilsAdjust.onTrackEvent(eventName);
    }

    public static void onTrackEvent(String eventName, String id) {
        AdUtilsAdjust.onTrackEvent(eventName, id);
    }

    public static void onTrackRevenue(String eventName, float revenue, String currency) {
        AdUtilsAdjust.onTrackRevenue(eventName, revenue, currency);
    }

    public static void onTrackRevenuePurchase(float revenue, String currency, String idPurchase, int typeIAP) {
        AdUtilsAdjust.onTrackRevenuePurchase(revenue, currency);
        AdUtilsAppsflyer.getInstance().onTrackRevenuePurchase(revenue, currency, idPurchase, typeIAP);
    }
    ...
    public static void pushTrackEventApplovin(MaxAd ad, Context context) {
        AdUtilsAdjust.pushTrackEventApplovin(ad, context);
    }
}
